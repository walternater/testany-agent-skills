...
description: Testany 流水线 CRUD - 创建/查询/更新/删除 pipeline，编排用例执行顺序和变量传递（执行请用 /testany-tests）
...
**注意**：
- 如果用户需要**执行** pipeline 或查看执行状态，请告知使用 `/testany-tests` 命令
- 如果用户需要创建**门禁**或**定时计划**，请告知使用 `/testany-orchestrator` 命令
...
**执行相关 (2)**
- `testany_execute_pipeline` - 执行 pipeline（建议使用 `/testany-tests`）
- `testany_get_pipeline_status` - 获取执行状态（建议使用 `/testany-tests`）

**注意**：执行相关工具虽然属于 pipeline 类别，但建议用户使用 `/testany-tests` skill 进行执行和监控操作。
...
| 场景 | 处理方式 |
|------|---------|
| 用户没有 case 但想创建 pipeline | 建议先使用 `/testany-case` 创建测试用例 |
...
| 想执行 pipeline | 告知用户使用 `/testany-tests` 命令 |
| 想创建定时计划或门禁 | 告知用户使用 `/testany-orchestrator` 命令 |
...
- 包含的 Case 数量和执行顺序
- Relay 配置摘要（如有）
- 下一步建议（如"可以使用 `/testany-tests` 执行 pipeline"或"可以使用 `/testany-orchestrator` 设置定时计划"）
