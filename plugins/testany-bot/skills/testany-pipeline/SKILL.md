...
- 如果用户需要创建**门禁**或**定时计划**，请切换到 `testany-orchestrator` workflow；如宿主支持 slash command，也可建议 `/testany-orchestrator`
...
| 想创建定时计划或门禁 | 建议切换到 `testany-orchestrator` workflow；如宿主支持 slash command，也可建议 `/testany-orchestrator` |
...
- 包含的 Case 数量和执行顺序
- Relay 配置摘要（如有）
- 下一步建议（如"可以切换到 `testany-tests` workflow 执行 pipeline"或"可以切换到 `testany-orchestrator` workflow 设置定时计划"）
