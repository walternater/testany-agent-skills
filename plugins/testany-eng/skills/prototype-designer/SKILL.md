...
3. **100% 复用前端仓库已有能力**：必须 import 仓库中已有的组件库、设计系统、路由方案、状态管理；禁止引入仓库外的 UI 框架或组件库
...
有缺口时：有近似组件则组合扩展，完全没有则用仓库样式方案在沙箱内创建最小化原型组件并标注 `[PROTOTYPE]`。**禁止引入仓库外的组件库。**
...
每个页面：组件组合（import 仓库组件）→ Mock 数据（对齐 PRD 业务实体）→ 状态实现（覆盖状态矩阵）→ 交互逻辑（按 Journey 步骤）。

代码规范：遵循仓库 lint/format 规则、import 约定、TypeScript 要求、样式方案。

Mock 数据集中放在沙箱目录的 `mock/` 下，包含正常数据、空数据、边界数据。
...
2. 无 script 但有 `tsconfig.json` → 使用对应包管理器的 exec 形式：`pnpm exec tsc --noEmit` / `yarn exec tsc --noEmit` / `bunx tsc --noEmit`
